...
# Default to chatty TensorFlow logging so progress is visible in notebooks/TPU runs
...
def run_cv(
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, strat_labels), 1):
        ...
        va_pred = model.predict(val_ds, verbose=1).reshape(-1)
        oof[va_idx] = va_pred
        test_pred += model.predict(test_ds, verbose=1).reshape(-1) / cfg.n_splits
...
def upload_model_to_kagglehub(
    ...
    """Upload a directory of model artefacts to Kaggle Models using kagglehub."""

    if not os.path.isdir(local_model_dir):
        raise FileNotFoundError(f"local_model_dir does not exist: {local_model_dir}")

    import kagglehub

    kagglehub.login()
    handle = f"{owner}/{model_slug}/{framework}/{variation_slug}"
    kagglehub.model_upload(
        handle=handle,
        local_model_dir=local_model_dir,
        version_notes=version_notes,
    )
    print(f"Uploaded {local_model_dir} to kaggle models: {handle}")
...
def parse_args() -> argparse.Namespace:
    ...
    train_parser.add_argument("--train_csv", type=str, default=None, help="Path to train.csv")
    train_parser.add_argument("--test_csv", type=str, default=None, help="Path to test.csv")
    ...
    train_parser.add_argument("--output_submission", type=str, default=None, help="Where to save the submission CSV")
    train_parser.add_argument("--epochs", type=int, default=None, help="Override epoch count (default from CFG)")
    train_parser.add_argument(
        "--batch_per_replica",
        type=int,
        default=None,
    ...
    upload_parser = subparsers.add_parser(
        "upload-model", help="Upload a directory of model artefacts to Kaggle Models via kagglehub"
    )
    upload_parser.add_argument("--local_model_dir", required=True, help="Directory containing model files to upload")
    upload_parser.add_argument("--owner", default="sicomaccapital", help="Kaggle username or org owner")
    upload_parser.add_argument("--model_slug", required=True, help="Model slug, e.g. `diabetes-tpu`")
    upload_parser.add_argument(
        "--variation_slug",
        default="default",
        help="Variation slug, e.g. `default` or `v1`. See https://www.kaggle.com/docs/models#name-model",
    )
    upload_parser.add_argument(
        "--framework",
        default="keras",
        choices=["keras", "tf", "pytorch", "sklearn", "xgboost"],
        help="Framework segment used in the model handle",
    )
    upload_parser.add_argument(
        "--version_notes",
        default="Update from tpu_diabetes_pipeline",
        help="Release notes for the uploaded version",
    )
...
def main():
    ...
    if args.command == "train":
        ...
        upload_model_to_kagglehub(
            local_model_dir=args.local_model_dir,
            owner=args.owner,
            model_slug=args.model_slug,
            framework=args.framework,
            variation_slug=args.variation_slug,
